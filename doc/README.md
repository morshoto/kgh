# Documentation

Detailed user documentation for `kgh` lives here.

## User Guides

- [Setup](./setup.md): prerequisites, install paths, and first-time environment setup
- [Config](./config.md): `.kgh/config.yaml` structure, example target, and field meanings
- [Local Run](./local-run.md): `kgh run`, dry-run vs live mode, and runtime overrides
- [GitHub Trigger](./github-trigger.md): commit-message trigger syntax and `kgh github run`

## Internal Context

- [PRD v1](./prd_v1.md): product and design context for the current Kaggle submit workflow
